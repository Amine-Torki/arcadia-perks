...
public final class PetsDashboardTab implements DashboardTabHandler {
    ...
    public void buildTab(SimpleContainer container, ServerPlayer player) {
        ...
        for (int i = 0; i < 36; i++) {
            ...
            if (colIndex < total) {
                ...
                lore.add(Component.translatable("arcadia_prestige.gui.pets.set_active").withStyle(ChatFormatting.AQUA));
                lore.add(Component.translatable("arcadia_prestige.gui.pets.retrieve").withStyle(ChatFormatting.YELLOW));
                ...
                setLore(empty, List.of(Component.translatable("arcadia_prestige.gui.pets.shift_deposit").withStyle(ChatFormatting.GRAY)));
        ...
        setName(pageInfo, Component.translatable("arcadia_prestige.gui.pets.page_info", petPage + 1, maxPage + 1).withStyle(ChatFormatting.WHITE));
    ...
    public ItemStack getNavBarItem(ServerPlayer player) {
        ...
        if (pd != null) {
            ...
            setLore(display, List.of(
                    Component.translatable("arcadia_prestige.gui.pets.active_label").withStyle(ChatFormatting.GOLD),
                    Component.translatable("arcadia_prestige.gui.pets.click_panel").withStyle(ChatFormatting.GREEN)));
        }
        return display;
    }
    ...
    public boolean handleInventoryShiftClick(Slot slot, ServerPlayer sp, Runnable refreshTab) {
        ...
        if (eq != null && eq.petId().equals(pd.petId())) {
            sp.sendSystemMessage(Component.translatable("arcadia_prestige.gui.pets.recall_first").withStyle(ChatFormatting.RED));
            return true;
        }
        ...
        if (col.size(sp.getUUID()) >= PetCollectionSavedData.MAX_PETS) {
            sp.sendSystemMessage(Component.translatable("arcadia_prestige.msg.collection_full").withStyle(ChatFormatting.RED));
        } else if (col.deposit(sp.getUUID(), clickedStack)) {
            slot.set(ItemStack.EMPTY);
            sp.sendSystemMessage(Component.translatable("arcadia_prestige.msg.pet_deposited").withStyle(ChatFormatting.GREEN));
            refreshTab.run();
        }
        return true;
    }
    ...
    private void withdrawPetToInventory(ServerPlayer sp, int colIndex, Runnable refreshTab) {
        ...
        if (sp.getInventory().getFreeSlot() < 0) {
            sp.sendSystemMessage(Component.translatable("arcadia_prestige.msg.inventory_full").withStyle(ChatFormatting.RED));
            return;
        }
        ...
        sp.sendSystemMessage(Component.translatable("arcadia_prestige.msg.pet_retrieved").withStyle(ChatFormatting.YELLOW));
